...
def is_valid_move(map, move):
    ...
    if  move[1] < 0 or  move[0] <0:
        return False 
    if move[1] < max_rows and move[0] < max_columns:
        return True
    return False
...
def result(map, hospital_coordinates, target_move):
    ...
    map1[target_move[1]][target_move[0]] = OBJECT_HOSPITAL
    map1[hospital_coordinates[1]][hospital_coordinates[0]] =  OBJECT_EMPTY
    return map1
...
def cost(map):
    ...
    """
    hospital = find_objects(map,OBJECT_HOSPITAL)

    house = find_objects(map,OBJECT_HOUSE)
